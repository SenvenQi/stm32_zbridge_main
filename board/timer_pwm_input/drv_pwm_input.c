 ...
 * Change Logs:
 * Date           Author       Notes
 * 2020-06-09     Roy.yu   first version
 */
...
#define PWM_INPUT4_CONFIG                     			\
    {                                           		\
		...
		.hdma_tim_ch1.Instance = DMA1_Stream0,			\
        .hdma_tim_ch1.Init.Channel = DMA_CHANNEL_2,		\
		.hdma_tim_ch2.Instance = DMA1_Stream3,			\
		.hdma_tim_ch2.Init.Channel = DMA_CHANNEL_2,		\
		.name = "pulse4"             					\
    }
...
rt_err_t pwm_input_init(rt_device_t pwm_input_device){
    ...
    stm32_device->tim_handler.Init.Prescaler = 840-1; //100k
